...
class EdgeUpdateLayer(MessagePassing):
    def __init__(self, edge_mlp):
        super(EdgeUpdateLayer, self).__init__(aggr='add')  # Use 'add' aggregation method

        # Define an MLP for updating edge features
        self.mlp=edge_mlp
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        node_attr: Node features
        """
        # Add self-loops to the adjacency matrix
        # edge_index, _ = add_self_loops(edge_index, num_nodes=x.size(0))

        # Compute normalization factor for each node
        row, col = edge_index
        ...
        # out = self.propagate(edge_index, x=x, edge_attr=edge_attr, node_attr=node_attr, norm=norm)
        out= self.edge_updater(edge_index, x=x, edge_attr=edge_attr)
        return out

    def message(self, x_i, x_j, edge_attr, norm):
        # Concatenate edge features, source node features, and target node features
        # print(x_i[-1].size(),x_j[-1].size(),edge_attr.size())
        msg_input = torch.cat([x_i, x_j, edge_attr], dim=-1)

        # Apply MLP to compute the new edge features
        updated_edge_attr = self.mlp(msg_input)

        return updated_edge_attr * norm.view(-1, 1)
    def edge_update(self, x_i,x_j, edge_attr):
        # Apply the edge update function
        return self.mlp(torch.cat([x_i, x_j, edge_attr], dim=-1))

    def update(self, aggr_out):
        # Aggregated messages are simply summed up
        return aggr_out
class TimeAwareNodeUpdate(MessagePassing):
    def __init__(self, flow_in_mlp, flow_out_mlp, node_mlp, node_aggr_func):
        super(TimeAwareNodeUpdate, self).__init__(aggr=None)  # Use 'add' aggregation method

        # Define an MLP for updating node features
        self.flow_in_mlp = flow_in_mlp
        self.flow_out_mlp = flow_out_mlp
        if node_aggr_func == 'mean':
            self.node_aggr_func = lambda out, row, x_size: scatter_mean(out, row, dim=0, dim_size=x_size)

        elif node_aggr_func == 'max':
            self.node_aggr_func = lambda out, row, x_size: scatter_max(out, row, dim=0, dim_size=x_size)[0]

        elif node_aggr_func == 'sum':
            self.node_aggr_func = lambda out, row, x_size: scatter_add(out, row, dim=0, dim_size=x_size)
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        # Perform message passing
        out = self.propagate(x=x, edge_attr=edge_attr, edge_index=edge_index)

        return out
    def message(self,x,edge_attr, edge_index):
        row, col = edge_index        
        # row, col = edge_index
        flow_out_mask = row < col
        flow_out_row, flow_out_col = row[flow_out_mask], col[flow_out_mask]
        ...
        flow_out = self.flow_out_mlp(flow_out_input)
        # flow_out = self.node_aggr_func(flow_out, flow_out_row, x.size(0))
        ...
        # flow_in = self.node_aggr_func(flow_in, flow_in_row, x.size(0))
        # flow=  torch.cat((flow_in, flow_out), dim=1)
        # return {"flow_in":flow_in,"flow_in_row":flow_in_row, "flow_out":flow_out, "flow_out_row":flow_out_row}
        return (x,flow_in,flow_in_row, flow_out,flow_out_row)
        
    def aggregate(self,inputs):
        x=inputs[0]
        flow_in=inputs[1]
        flow_in_row=inputs[2]
        flow_out=inputs[3]
        flow_out_row=inputs[4]
        
        f_out = self.node_aggr_func(flow_out, flow_out_row, x.size(0))
        f_in = self.node_aggr_func(flow_in, flow_in_row, x.size(0))
        flow=  torch.cat((f_in, f_out), dim=1)
...
class MOTMPNet(nn.Module):
    def __init__(self, model_params, bb_encoder = None):
        ...
        self.model_params = model_params
        # Define Encoder and Classifier Networks
        encoder_feats_dict = self.model_params['encoder_feats_dict']
        classifier_feats_dict = self.model_params['classifier_feats_dict']
        # encoder_layers=[]
        # classifier_layers=[]
        # self.node_encoder=nn.Sequential(nn.Linear(encoder_feats_dict['node_in_dim']),
                                         
        #                                 )
        self.NodeEncoder = self.build_mlp(encoder_feats_dict['node_in_dim'], list(encoder_feats_dict['node_fc_dims'])+[encoder_feats_dict['node_out_dim']], encoder_feats_dict['dropout_p'], encoder_feats_dict['use_batchnorm'])
        self.EdgeEncoder=self.build_cnn(encoder_feats_dict['edge_num_conv_layers'],encoder_feats_dict['edge_kernal_size'], encoder_feats_dict['edge_padding'],encoder_feats_dict['edge_in_dim'], list(encoder_feats_dict['edge_fc_dims'])+[encoder_feats_dict['edge_out_dim']], encoder_feats_dict['dropout_p'], encoder_feats_dict['use_batchnorm'])
        # self.classifier =self.build_mlp(classifier_feats_dict['edge_in_dim'], list(classifier_feats_dict['edge_fc_dims'])+[1], classifier_feats_dict['dropout_p'], classifier_feats_dict['use_batchnorm'])
        self.classifier=self.build_cnn(classifier_feats_dict['num_conv_layers'], classifier_feats_dict['kernal_size'], classifier_feats_dict['padding'], classifier_feats_dict['edge_in_dim'], list(classifier_feats_dict['edge_fc_dims'])+[1], classifier_feats_dict['dropout_p'], classifier_feats_dict['use_batchnorm'])
        # Define the 'Core' message passing network (i.e. node and edge update models)
        self.EdgeUpdateLayer, self.NodeUpdateLayer = self._build_core_MPNet(encoder_feats_dict=encoder_feats_dict)

        self.num_enc_steps = self.model_params['num_enc_steps']
        self.num_class_steps = self.model_params['num_class_steps']
    def build_cnn(self, num_conv_layers, kernal_size, padding,input_dim, fc_dims, dropout_p=0.4, use_batchnorm=False):
        layers=[]
        for i in range(num_conv_layers):
            layers.append(nn.Conv1d(in_channels=1, out_channels=1, kernel_size=kernal_size, padding=padding))
        for dim in fc_dims:
    ...
    def build_mlp(self, input_dim, fc_dims, dropout_p=0.4, use_batchnorm=False):
        layers = []
        for dim in fc_dims:
            layers.append(nn.Linear(input_dim, dim))
            if use_batchnorm and dim != 1:
    ...
    def _build_core_MPNet(self, encoder_feats_dict):
        ...
        # Define an aggregation operator for nodes to 'gather' messages from incident edges
        node_agg_fn = self.model_params['node_agg_fn']
        assert node_agg_fn.lower() in ('mean', 'max', 'sum'), "node_agg_fn can only be 'max', 'mean' or 'sum'."
        ...
        # For both nodes and edges, the initial encoded features (i.e. output of self.encoder) can either be
        # reattached or not after each Message Passing Step. This affects MLPs input dimensions
        self.reattach_initial_nodes = self.model_params['reattach_initial_nodes']
        self.reattach_initial_edges = self.model_params['reattach_initial_edges']
        ...
        # Define all MLPs used within the MPN
        edge_model_feats_dict = self.model_params['edge_model_feats_dict']
        node_model_feats_dict = self.model_params['node_model_feats_dict']

        edge_mlp = self.build_mlp(input_dim=edge_model_in_dim,
                       fc_dims=edge_model_feats_dict['fc_dims'],
                       dropout_p=edge_model_feats_dict['dropout_p'],
                       use_batchnorm=edge_model_feats_dict['use_batchnorm'])

        flow_in_mlp =self.build_mlp(input_dim=node_model_in_dim,
                          fc_dims=node_model_feats_dict['fc_dims'],
                          dropout_p=node_model_feats_dict['dropout_p'],
                          use_batchnorm=node_model_feats_dict['use_batchnorm'])

        flow_out_mlp = self.build_mlp(input_dim=node_model_in_dim,
                           fc_dims=node_model_feats_dict['fc_dims'],
                           dropout_p=node_model_feats_dict['dropout_p'],
                           use_batchnorm=node_model_feats_dict['use_batchnorm'])
        ...
        # Define all MLPs used within the MPN
        return EdgeUpdateLayer(edge_mlp=edge_mlp), TimeAwareNodeUpdate(flow_in_mlp=flow_in_mlp, flow_out_mlp=flow_out_mlp, node_mlp=node_mlp, node_aggr_func=node_agg_fn)
    ...
    def forward(self, data):
        ...
        # Encoding features step
        latent_node_feats = self.NodeEncoder(x)
        edge_attr_reshaped=edge_attr.reshape(edge_attr.size()[0], 1, edge_attr.size()[-1])
        latent_edge_feats=self.EdgeEncoder(edge_attr_reshaped).squeeze()
        initial_edge_feats = latent_edge_feats
        initial_node_feats = latent_node_feats
        ...
        for step in range(1, self.num_enc_steps + 1):
            ...
            # Message Passing Step
            latent_edge_feats = self.EdgeUpdateLayer(latent_node_feats, edge_index, latent_edge_feats)
            latent_node_feats=self.NodeUpdateLayer(latent_node_feats, edge_index, latent_edge_feats)

            if step >= first_class_step:
                # Classification Step
                # print(latent_edge_feats)
                # print(latent_edge_feats.size())
                
                # input()
                # for i in range(len(latent_edge_feats)):
                #     dec_edge_feats.append(self.classifier(latent_edge_feats[i]))
                # # dec_edge_feats
                reshaped_edge_feats=latent_edge_feats.reshape(latent_edge_feats.size()[0], 1, latent_edge_feats.size()[-1])
                dec_edge_feats = self.classifier(reshaped_edge_feats)
                # print(dec_edge_feats)
                # input()
                outputs_dict['classified_edges'].append(dec_edge_feats)

        if self.num_enc_steps == 0:
            dec_edge_feats = self.classifier(latent_edge_feats)
            outputs_dict['classified_edges'].append(dec_edge_feats)

        # return latent_node_feats, latent_edge_feats    
        return outputs_dict    
