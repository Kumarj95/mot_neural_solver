...
class MOTMPNet(nn.Module):
    def __init__(self, model_params, bb_encoder = None):
        ...
        self.EdgeEncoder=self.build_mlp(encoder_feats_dict['edge_in_dim'], list(encoder_feats_dict['edge_fc_dims'])+[encoder_feats_dict['edge_out_dim']], encoder_feats_dict['dropout_p'], encoder_feats_dict['use_batchnorm'])
        ...
        self.classifier=self.build_cnn(classifier_feats_dict['num_conv_layers'], classifier_feats_dict['edge_in_dim'], list(classifier_feats_dict['edge_fc_dims'])+[1], classifier_feats_dict['dropout_p'], classifier_feats_dict['use_batchnorm'])
        # Define the 'Core' message passing network (i.e. node and edge update models)
    ...
    def build_cnn(self, num_conv_layers,input_dim, fc_dims, dropout_p=0.4, use_batchnorm=False):
        layers=[]
        for i in range(num_conv_layers):
            layers.append(nn.Conv1d(in_channels=1, out_channels=1, kernel_size=3, padding=1))
        for dim in fc_dims:
    ...
    def forward(self, data):
        ...
        latent_edge_feats=self.EdgeEncoder(edge_attr)
        initial_edge_feats = latent_edge_feats
        initial_node_feats = latent_node_feats
